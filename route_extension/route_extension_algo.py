...
class ESDComputer:
    ...
    def compute_ESD_in_horizon(self, station_id: int, t_arr: int, ins: int, x_s_arr: list,
                               x_c_arr: list, mode: str, delta: bool) -> float:
        """
    ...
    def compute_route(self, r: list, t_left: int, init_l: int, x_s_arr: list, x_c_arr: list):
        ...
        reward_arr, trace_arr = np.full((level_num, station_num), minus_M, dtype=float), np.full((level_num, station_num),
                                                                                    minus_M)
        if route[0] == 0:  # starting from depot, only load bikes
            ...
            for j in range(level_num):
                ...
                if 0 <= round(x_s_0 + ins) <= cap_station:
                    reward_arr[j, 0] = self.compute_ESD_in_horizon(station_id=route[0], t_arr=t_left, ins=ins,
                                                                   x_s_arr=x_s_arr, x_c_arr=x_c_arr, mode='multi',
                                                                   delta=False)
            ...
            if self.re_start_t + self.t_cur + t_trip <= self.re_end_t:
                    ...
                    for former_k in range(level_num):
                        if reward_arr[former_k, i - 1] == false_flag:  # infeasible
                            ...
                            if 0 <= round(self.ei_s_arr[
                                ...
                                station_esd = self.compute_ESD_in_horizon(station_id=route[i], t_arr=t_trip, ins=ins,
                                                                          x_s_arr=x_s_arr, x_c_arr=x_c_arr, mode='multi',
                                                                          delta=False)
                ...
                for k in range(level_num):
                    ...
                    if reward_arr[former_k, i - 1] == false_flag:  # infeasible
                        ...
                        station_esd = self.compute_ESD_in_horizon(station_id=route[i], t_arr=0, ins=0,
                                                                  x_s_arr=x_s_arr, x_c_arr=x_c_arr, mode='multi',
                                                                  delta=False)
                        if station_esd + reward_arr[former_k, i - 1] > reward_arr[k, i]:
                            reward_arr[k, i] = station_esd + reward_arr[former_k, i - 1]
    ...
    def get_on_route_time(self, route: list) -> int:
        ...
        for i in range(1, len(route)):
            t_trip += (self.c_mat[route[i - 1], route[i]] - 1 if route[i - 1] != 0 else self.c_mat[route[i - 1], route[i]])
        return round(t_trip)
...
def get_REA_routes_test(num_of_van: int, van_location: list, van_dis_left: list, van_load: list,
                        ...
                        alpha: float, est_ins: int, branch: int = 2) -> dict:
    """
    ...
    for van in range(num_of_van):
        ...
        while undone_nodes:
            ...
            if est_load / c_v < DELTA_CAP:  # relatively few bikes
                    ...
                    for idx in range(len(max_index)):
                        ...
                        assert (0 <= opt_ins +
                                round(ei_s_arr[
                                    option - 1,
                                    reg_cur_t,
                                    round(reg_cur_t + node_t + c_mat[node_station, option]) if round(reg_cur_t + node_t + c_mat[node_station, option]) < 49 else 48,
                                    x_s_arr[option - 1],
                ...
                exp_inv = [
                    round(ei_s_arr[i,
                                   reg_cur_t,
                                   round(reg_cur_t + node_t + c_mat[node_station, i + 1]) if (reg_cur_t + node_t + c_mat[node_station, i + 1]) < 49 else 48,
                                   x_s_arr[i],
                                   x_c_arr[i]])
                    for i in range(num_of_stations)]
                for j in ant_stations:
                ...
                #         else:
                #             arr_a, arr_b = (
                #                 node_t + c_mat[node_station, a], node_t + c_mat[node_station, a] + c_mat[a, b])
                #             pair_ins_list, pair_profit_list = [], []
                #             for q_a in range(est_load):
                #                 q_b = est_load - q_a
                #                 ins_a = min(q_a, U_REA - exp_inv[a - 1])
                #                 ins_b = min(q_b, U_REA - ei_s_arr[b - 1, reg_cur_t, round(reg_cur_t + arr_b), x_s_arr[b - 1], x_c_arr[b - 1]])
                #                 profit_a = esd_computer.compute_ESD_in_horizon(
                #                     station_id=a, t_arr=arr_a, ins=ins_a, x_s_arr=x_s_arr,
                #                     x_c_arr=x_c_arr, mode='multi', delta=True)
                #                 profit_b = esd_computer.compute_ESD_in_horizon(
                #                     station_id=b, t_arr=arr_b, ins=ins_b, x_s_arr=x_s_arr,
                #                     x_c_arr=x_c_arr, mode='multi', delta=True)
                #                 profit_per_min = (profit_a + profit_b) / (c_mat[node_station, a] + c_mat[a, b])
                #                 pair_ins_list.append((ins_a, ins_b))
                #                 pair_profit_list.append(profit_per_min)
                #             _max_profit = max(pair_profit_list)
                #             _max_profit_ins = pair_ins_list[pair_profit_list.index(_max_profit)]
                #             visit_pair.append((a, b))
                #             visit_ins.append(_max_profit_ins)
                #             visit_profit_per_min.append(_max_profit)
                # compare and choose
        ...
        for node in done_nodes:
            ...
            last_1, last_2, last_3 = node.route[-1], node.route[-2], node.route[-3]
            if node.ins < 0:  # most recent action is loading
                ...
                if end_node not in end_node_list:
                    ...
                    profit_list.append(cur_profit - alpha * on_route_t)
        ...
        improve_list, improved_profit_list, positive_count = [], [], 0
        ...
        best_profits = heapq.nlargest(5, profit_list)
        print('five best routes: ', best_profits)
